...
#if (BLE_BXOTA_SERVER)
extern const struct prf_task_cbs* bxotas_prf_itf_get(void);
#endif

#if (BLE_BXOTA_CLINET)
extern const struct prf_task_cbs* bxotac_prf_itf_get(void);
#endif
...
#if (BLE_CTS_CLIENT)
extern const struct prf_task_cbs *ctsc_prf_itf_get(void);
#endif

#if (BLE_GATTS_CLIENT)
extern const struct prf_task_cbs *gattsc_prf_itf_get(void);
#endif

#if (BLE_USER_PROFILE )
extern const struct prf_task_cbs *user_profile_prf_itf_get(void);
#endif
/*
 * TYPE DEFINITIONS
 ****************************************************************************************
...
static const struct prf_task_cbs * prf_itf_get(uint16_t task_id)
{
    ...
    switch(KE_TYPE_GET(task_id))
    {
        ...
        #if (BLE_BXOTA_SERVER)
        case TASK_ID_BXOTAS:
            prf_cbs = bxotas_prf_itf_get();
            break;
        #endif // (BLE_BXOTA_SERVER)

        #if (BLE_BXOTA_CLIENT)
        case TASK_ID_BXOTAC:
            prf_cbs = bxotac_prf_itf_get();
            break;
        #endif
        ...
        #if (BLE_GATTS_CLIENT)
        case TASK_ID_GATTSC:
            prf_cbs = gattsc_prf_itf_get();
            break;
        #endif
        
        #if (ALM_PROFILE)
        case TASK_ID_ALMPROFILE:
            prf_cbs = user_profile_prf_itf_get();
            break;
        #endif
